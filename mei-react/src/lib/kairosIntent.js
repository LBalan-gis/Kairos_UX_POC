...
export const KNOWLEDGE = [
  ...
  { match:['otif','on-time in-full','on time','friday','delivery','dispatch','sap schedule','erp schedule','ship','batch completion','miss target'],
    ...
    focusIds:['planned_vs_actual','impact_yield','sim_corrected'], chartKey:'planned',
...
export const GRAPH_CMDS = ['drift path','root cause','simulate correction','explain deviation','show affected systems','cross line','golden batch','tension impact','hidden loss','micro-stop'];

// ─── Machine label → equipment id map ────────────────────────────────────────
export const MACHINE_ID_MAP = {
...
export function resolveMachineId(text) {
...
export const MACHINE_LABELS = Object.fromEntries(
  Object.entries(MACHINE_ID_MAP).map(([, id]) => [id, id.replace('_','-').toUpperCase()])
);

// ─── Intent detection ─────────────────────────────────────────────────────────
// currentPanel is passed explicitly (was previously closed over from state)
export function detectIntent(text, currentPanel) {
  const t = text.toLowerCase().trim();
  // Panel modification — check first, most specific
  if (currentPanel) {
  ...
  // Feature 2: AEGIS — hallucination trap (fabrication attempt)
  ...
  // Feature 3: CFR block — override/force-offline attempt
