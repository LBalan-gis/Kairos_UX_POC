...
import { gateWidget } from './widget/WidgetGate';
import { BlockedWidget } from './widget/WidgetPrimitives';
import { WidgetMount } from './widget/WidgetMount';
...
export function WidgetRenderer({
  ...
  const resolvedProps = (rawProps.live && rawProps.tag)
    ? { ...rawProps, value: liveDataProvider(rawProps.tag) ?? rawProps.value }
    : rawProps;
  ...
  const componentProps = {
    ...
    onAction: entry.requiresAuth
      ? (data) => onAction?.({ ...data, requiresAuth: true, widgetType: payload.type })
      : onAction,
  };
