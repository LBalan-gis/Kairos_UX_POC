...
// Pill marker colours — Google Maps badge palette
const PILL_PAST   = '#546E7A'; // muted steel blue-grey — historical markers
const PILL_FUTURE = '#29B6C8'; // Maps sky-teal — prediction zone markers
const PILL_NOW_BG = '#FFFFFF'; // opaque white — most prominent
const PILL_NOW_FG = '#111111';

// Panel
const PANEL_BG     = 'rgba(22, 30, 42, 0.72)';
const PANEL_BORDER = 'rgba(255,255,255,0.22)';
...
export function PredictionTimeline() {
  ...
  // All scenario future bands stacked
  ...
  // Tick marks: major every 10 min, minor every 5 min
  const ticks = useMemo(() => {
    const out = [];
  ...
  return (
    <div style={{
      ...
      {/* ── Collapsed toggle — right side, aligned with other controls ── */}
      {!open && (
      ...
      {/* ── Floating pane — slides in from right ── */}
      <div style={{
        margin: '10px 14px',
        background: PANEL_BG,
        ...
        {/* ── Top row: scenario badges + scrub readout + live ── */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
          ...
          {/* Collapse — rightmost, always last */}
        ...
        {/* ── Timeline track + pill markers ── */}
        <div style={{ position: 'relative' }}>

          {/* Pill markers row — sits above the track */}
          <div style={{ position: 'relative', height: 22, marginBottom: 4 }}>
            {ticks.map(({ t, major }) => {
              if (!major) return null;
              // Skip edge markers to prevent overflow
            ...
            {/* Minor tick lines between major pills */}
