...
export function WidgetMount({
  ...
  isChild,
  dark,
  renderChild,          // (child: payload, i: number) => ReactNode — from WidgetRenderer
}) {
  const isLayout      = payload.type === 'layout';
  const Component     = entry?.component;
  const hasChildren   = payload.children?.length > 0;
  const childDirection = payload.props?.direction ?? 'column';
  ...
  const amberSubLabel = namespaceAmber
    ? `tag "${payload.props?.tag}" is not registered`
