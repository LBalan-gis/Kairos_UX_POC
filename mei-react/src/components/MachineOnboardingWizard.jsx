...
function StepIdentity({ form, setForm, s }) {
  return (
...
function StepDocs({ docs, setDocs, s }) {
...
function StepTags({ machineType, tags, setTags, parsing, setTagPrefix, s }) {
  if (parsing) {
    return (
...
function StepMapping({ tags, setTags, s }) {
...
function StepPhysics({ tags, setTags, s }) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      {enabledAI.map((tag, i) => (
        <div key={i} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr', alignItems: 'center', gap: 0, borderBottom: `1px solid ${s.borderWeak}` }}>
          ...
          {['setpoint','lolo','lo','hi','hihi'].map(field => (
...
function StepReview({ form, docs, tags, s }) {
  const enabledTags = tags.filter(t => t.enabled !== false);
...
function ScannerScreen({ onConfirm, onManual }) {
        ...
        <div style={{ display: 'flex', flex: 1, overflow: 'hidden', gap: 0 }}>
          ...
          <div style={{ width: 220, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.40)', borderRight: '1px solid rgba(42,241,229,0.10)' }}>
            <div style={{ position: 'relative', width: 140, height: 140 }}>
              {/* Rings */}
              {[1, 0.67, 0.33].map((s, i) => (
                <div key={i} style={{ position: 'absolute', inset: 0, borderRadius: '50%', border: '1px solid rgba(42,241,229,0.14)', transform: `scale(${s})`, transformOrigin: 'center' }} />
              ))}
              {/* Cross-hairs */}
            ...
            >
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                <div style={{ flex: 1, display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '4px 12px' }}>
                  {[['ID', MOCK_DISCOVERED.id], ['Type', MOCK_DISCOVERED.type], ['Manufacturer', MOCK_DISCOVERED.manufacturer], ['Model', MOCK_DISCOVERED.model], ['Serial', MOCK_DISCOVERED.serial], ['Line', MOCK_DISCOVERED.line]].map(([k, v]) => (
                    <div key={k}>
...
export function MachineOnboardingWizard() {
  ...
  const [scanPhase, setScanPhase] = useState('scanning'); // 'scanning' | null

  const [form, setForm] = useState({ id:'', serial:'', type:'', manufacturer:'', model:'', line:'', ccRef:'' });
  ...
  const handleScanConfirm = (discovered) => {
    setForm(f => ({
      ...f,
