...
export function KairosCFRGate() {
  ...
  const setMachineOffline = useAppStore((state) => state.setMachineOffline);
            ...
            <div style={{ padding: '18px 20px 20px', display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div>
                ...
                <input
                  ...
                  value={cfrPin}
                  onChange={(event) => {
                    setCfrPin(event.target.value);
                    setCfrError(false);
                  }}
                  onKeyDown={(event) => {
                    if (event.key === 'Enter') document.getElementById('cfr-authorize-btn')?.click();
                  }}
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <button
                  ...
                  style={{ flex: 1, padding: '9px 0', borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: 'pointer', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', color: 'rgba(255,255,255,0.50)' }}
                >
                  Cancel
                </button>
                <button
                  id="cfr-authorize-btn"
                  onClick={() => {
                    if (cfrPin.toUpperCase() === 'ASTELLAS' || cfrPin === '1234') {
                      ...
                      const auditId = `KR-${new Date().toISOString().slice(0, 10)}-${Math.floor(Math.random() * 9000 + 1000)}`;
                      setMessages((prev) => [
                        ...prev,
                        mkMsg('system', { text: `CFR-11 gate cleared · ${cfrPending.label} → OFFLINE · audit record ${auditId} written · operator-signed` }),
                        mkMsg('kairos', { text: `Override authorized and executed. ${cfrPending.label} is now offline. Audit record written to GxP trail — operator-signed, timestamped, immutable.`, actions: [] }),
                      ]);
                      setCfrPending(null);
                      setCfrPin('');
                      setCfrError(false);
