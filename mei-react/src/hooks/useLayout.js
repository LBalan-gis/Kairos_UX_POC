...
export function estimateSize(entity) {
  ...
  // Use slightly generous heights to definitively dodge initial rendering overlaps.
  ...
  return {
    ...
    height: entity.chart ? 240 : 200, // generously static layout padding guarantees no snap-collision on reveal
  };
}
...
const INTER_ZONE_GAP = 110; // matches reflowZones MIN_GAP — HTML uses 110
const INTRA_ZONE_GAP = 28; // matches HTML INTRA_GAP
const MARGIN_TOP     = 120; // Increased to leave room for Swimlane headings

export function useLayout(entities, zoneMap) {
  return useMemo(() => {
    if (!entities?.length) return { positions: {}, sizes: {} };

    // Compute estimated sizes for all entities
    const sizes = {};
    entities.forEach((e) => { sizes[e.id] = estimateSize(e); });

    // Group entities by zone; track max width per zone
    const zoneGroups = {};
    const zoneMaxW   = {};
    entities.forEach((e) => {
    ...
    // Compute zone column X positions left-to-right (mirrors HTML cursorX logic)
    const zoneStartX = {};
    ...
    // Stack entities in each zone by rankHint (HTML sorts by dagre Y → same intent)
    const positions = {};
    present.forEach((zone) => {
...
export function computeFitTransform(positions, sizes, viewW, viewH = window.innerHeight) {
