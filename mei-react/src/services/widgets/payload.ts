...
function topBound(entityId = 'vis_1101'): WidgetPayload['spatialBinding'] {
  return { entityId, anchor: 'top' };
}

export function buildWidgetPayload(text: string): WidgetPayload {
  ...
  if (tl.includes('gauge') || tl.includes('oee gauge')) {
    return {
      type: 'gauge',
      confidence: 0.93,
  ...
  if (tl.includes('sparkline') || tl.includes('trend') || tl.includes('tension')) {
    return {
      type: 'chart',
      confidence: 0.91,
      props: {
  ...
  if (tl.includes('line status') || tl.includes('table')) {
    return {
      type: 'table',
      confidence: 0.89,
      props: {
  ...
  if (tl.includes('adjust tension') || tl.includes('throttle') || tl.includes('execute action')) {
    return {
      type: 'action',
      confidence: 0.97,
      props: { label: 'Adjust FT-1101 Tension Roller → 42 N', action: 'adjust_tension_ft1101', machineId: 'bf_1101', risk: 'medium' },
  ...
  if (tl.includes('switch') || tl.includes('toggle')) {
    return {
      type: 'control',
      confidence: 0.90,
  ...
  if (tl.includes('slider') || tl.includes('set speed') || tl.includes('setpoint')) {
    return {
      type: 'control',
      confidence: 0.88,
  ...
  if (tl.includes('radio') || tl.includes('mode') || tl.includes('speed mode')) {
    return {
      type: 'control',
      confidence: 0.87,
  ...
  if (tl.includes('checkbox') || tl.includes('check')) {
    return {
      type: 'control',
      confidence: 0.85,
  ...
  if (tl.includes('calendar') || tl.includes('date') || tl.includes('schedule')) {
    return {
      type: 'control',
      confidence: 0.84,
  ...
  if (tl.includes('clock') || tl.includes('time') || tl.includes('shift start')) {
    return {
      type: 'control',
      confidence: 0.84,
  ...
  if (tl.includes('form') || tl.includes('input') || tl.includes('control')) {
    return {
      type: 'control',
      confidence: 0.86,
  ...
  if (tl.includes('composable') || tl.includes('monitor panel') || tl.includes('layout')) {
    return {
      type: 'layout',
      confidence: 0.93,
