...
export function WidgetControl({ controlType = 'switch', label, value, options, min, max, step, action, machineId, onAction, dark = true }) {
  ...
  const accentOn    = '#2AF1E5';
  const accentOnL   = '#0284C7';
  const accent      = dark ? accentOn : accentOnL;
  ...
  switch (controlType) {
      ...
      return wrap(
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <input type="range" min={min} max={max} step={step} value={val}
            ...
            style={{ flex: 1, accentColor: accent, cursor: 'grab' }} />
          <div style={{ padding: '4px 8px', background: inputBg, borderRadius: 4, border: `1px solid ${inputBorder}`, minWidth: 44, textAlign: 'center' }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: accent, fontVariantNumeric: 'tabular-nums' }}>{val}</span>
          </div>
        </div>
      );
    ...
    case 'calendar':
      return wrap(
        <input type="date" value={val} onChange={e => handleChange(e.target.value)}
          style={{ padding: '10px 14px', background: inputBg, border: `1px solid ${inputBorder}`, color: inputColor, borderRadius: 6, outline: 'none', cursor: 'text', colorScheme }} />
      );

    case 'clock':
      return wrap(
        <input type="time" value={val} onChange={e => handleChange(e.target.value)}
          style={{ padding: '10px 14px', background: inputBg, border: `1px solid ${inputBorder}`, color: inputColor, borderRadius: 6, outline: 'none', cursor: 'text', colorScheme }} />
      );
    ...
    case 'checkbox': {
      ...
      const checkBox = (checked, label, onToggle) => (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }} onClick={onToggle}>
          ...
          <span style={{ fontSize: 11, color: checked ? textColor : dimColor, fontWeight: checked ? 600 : 400 }}>{label}</span>
        </div>
      );
    ...
    case 'select':
      return wrap(
        <select value={val} onChange={e => handleChange(e.target.value)}
          style={{ padding: '10px 14px', background: inputBg, border: `1px solid ${inputBorder}`, color: inputColor, borderRadius: 6, outline: 'none', cursor: 'pointer', width: '100%', colorScheme }}>
    ...
    case 'text':
      return wrap(
        <input type="text" value={val} placeholder={options?.[0] ?? ''} onChange={e => handleChange(e.target.value)}
    ...
    case 'textarea':
      return wrap(
        <textarea value={val} rows={4} placeholder={options?.[0] ?? ''} onChange={e => handleChange(e.target.value)}
    ...
    case 'badge': {
      ...
      const [bg, border, color] = BADGE[val] ?? BADGE.info;
      return (
        <div style={{ padding: '14px 16px' }}>
