...
function getBaseOptions(yMin, yMax, yUnit = "") {
  return {
    responsive: true,
    maintainAspectRatio: false,
    animation: false, // Matches their fast-render monolith feel
    ...
    scales: {
      ...
      y: {
        ...
        grid: { color: THEME.grid, drawTicks: false },
        ticks: { 
          color: THEME.axis, maxTicksLimit: 5, font: { size: 11, weight: "600" }, 
          callback: (v) => {
            const formatted = Number.isInteger(v) 
              ? v.toLocaleString('en-US') 
              : Number(v).toLocaleString('en-US', { maximumFractionDigits: 2 });
...
export function MetricSparkline({ entityId, simulatedTime }) {
  const chartConfig = useMemo(() => {
    const isSim = simulatedTime !== null && simulatedTime > 0;
    // Map 0-60min simulation onto 24 graph data points
